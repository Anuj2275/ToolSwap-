...
const MyToolsPage = () => {
    ...
    const handleDelete = async (toolIdToDelete) => {
        ...
        try {
            ...
            setMyTools(prevTools => prevTools.filter(tool => tool.id !== toolIdToDelete));
            toast.success('Tool deleted successfully!')
        } catch (err) {
            toast.err(`Failed to delete tool: ${err.response?.data?.message || err.message}`);            
...
return (
    <div className="space-y-8">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
            <h1 className="text-3xl font-bold text-text-light dark:text-text-dark mb-3 sm:mb-0">My Tools</h1>
             <Link
                to="/add-tool"
        ...
        {myTools.length > 0 ? (
            <div className="bg-card-light dark:bg-card-dark rounded-xl shadow-xl border border-border-light dark:border-border-dark overflow-hidden">
                <ul className="divide-y divide-border-light dark:divide-border-dark">
                    {myTools.map((tool) => {
                         const toolImageUrl = tool.imageUrl && tool.imageUrl.startsWith('http') ? tool.imageUrl : null;
                        return (
                            <li key={tool.id} className="p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors"> {/* Adjusted gap/padding */}
                                <div className="flex items-center gap-4 flex-grow min-w-0">
                                     {toolImageUrl ? (
                                        <img src={toolImageUrl} alt={tool.name} className='w-16 h-16 object-cover rounded-lg flex-shrink-0 shadow-sm' onError={(e) => e.target.style.display='none'} />
                                    ) : (
                                        <div className="w-16 h-16 bg-gray-200 dark:bg-slate-600 rounded-lg flex items-center justify-center text-xs text-muted-light dark:text-muted-dark flex-shrink-0 shadow-sm"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}> <path strokeLinecap="round" strokeLinejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg></div>
                                    )}
                                     <div className='overflow-hidden'>
                                        ...
                                        <p className="text-sm text-muted-light dark:text-muted-dark mt-0.5">{tool.category || 'Uncategorized'}</p>
                                    </div>
                                </div>
             ...
             <div className="bg-card-light dark:bg-card-dark p-10 rounded-xl shadow-xl border border-border-light dark:border-border-dark text-center">
                <svg xmlns="http://www.w3.org/2000/svg" className="mx-auto h-12 w-12 text-muted-light dark:text-muted-dark" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}> <path strokeLinecap="round" strokeLinejoin="round" d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4M4 7c0-.53.181-1.03.504-1.445M20 7c0-.53-.181-1.03-.504-1.445M12 11V3m0 18v-8" /></svg> {/* Database Icon */}
                <p className="mt-4 text-muted-light dark:text-muted-dark">You haven't listed any tools yet.</p>
                 <Link
                    to="/add-tool"
