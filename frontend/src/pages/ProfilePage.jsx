...
const AvatarPlaceholder = ({ name, size = 'large' }) => {
    const sizeClasses = {
        ...
        large: 'w-24 h-24 text-3xl', // Made larger for profile page
    };
    const initial = name ? name.charAt(0).toUpperCase() : '?';
    return (
        <div className={`relative rounded-full bg-gradient-to-br from-teal-400 to-cyan-500 dark:from-teal-500 dark:to-cyan-600 flex items-center justify-center text-white font-semibold shadow-md ${sizeClasses[size]}`}>
            {initial}
...
const ProfilePage = () => {
    ...
    // --- NEW LUXURIOUS UI START ---
    return (
        <div className="max-w-md mx-auto mt-8 md:mt-12"> {/* Centered, smaller max-width */}
            <div className="bg-card-light dark:bg-card-dark rounded-xl shadow-xl overflow-hidden border border-border-light dark:border-border-dark p-6 md:p-8 transform transition-all duration-300 hover:shadow-2xl"> {/* Added hover shadow */}
                {/* Profile Header */}
                <div className="flex flex-col items-center mb-8 text-center">
                    <AvatarPlaceholder name={user.name} size="large" />
                    <h1 className="mt-5 text-2xl font-bold text-text-light dark:text-text-dark">{user.name}</h1>
                    <p className="mt-1 text-sm text-muted-light dark:text-muted-dark">{user.email}</p>
                </div>
